...
export default function CurriculumPreview({ curriculum }: { curriculum: CurriculumYear[] }) {
  ...
  // Global semester index across all years for consistent gradient assignment
  let semOffset = 0
      ...
      <div key={activeYear} className="grid grid-cols-1 sm:grid-cols-2 gap-5 animate-fade-in">
        {current.semesters.map((sem, si) => {
          ...
          return (
            ...
            >
              {/* Coloured header */}
              <div className="px-5 py-4 flex items-center justify-between" style={{ background: SEM_GRADS[gi] }}>
              ...
              <div className="bg-white p-5">
                <ul className="flex flex-col gap-2.5">
                  {sem.subjects.map((subj, ji) => (
                    <li key={subj} className="flex items-start gap-3 text-[13px] font-body text-neutral-700">
                      <span
