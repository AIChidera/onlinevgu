...
export default function FeesScholarships({ totalFee, duration, feeBreakdown, scholarshipTiers }: Props) {
  return (
    <section className="bg-white pt-8 pb-16 px-5 md:px-8 lg:px-12 border-t border-neutral-100">
      <div className="mx-auto max-w-[1280px]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div data-animate="fade-up" className="rounded-2xl border border-neutral-200 bg-white overflow-hidden">
            <div className="px-4 py-4 md:px-6 border-b border-neutral-100 bg-neutral-50/60 flex items-center gap-3">
            ...
            <div className="divide-y divide-neutral-100">
              ...
              <div className="flex items-center justify-between px-4 py-4 md:px-6 bg-neutral-50">
          ...
          <div data-animate="fade-up" style={{ animationDelay: '80ms' }} className="rounded-2xl border border-neutral-200 bg-white overflow-hidden">
            ...
            <div className="divide-y divide-neutral-100">
              {scholarshipTiers.map(t => (
                <div key={t.criteria} className="flex items-start gap-4 px-4 py-3.5 md:px-6">
                  <p className="flex-1 text-[16px] font-body text-neutral-700 leading-[1.5]">{t.criteria}</p>
...
function FeeRow({ label, value, note }: { label: string; value: string; note?: string }) {
  return (
    <div className="flex items-center justify-between px-4 py-3.5 md:px-6">
      <div>
        <p className="text-[16px] font-body text-neutral-700">{label}</p>
        {note && <p className="text-[13px] font-body text-neutral-400 mt-0.5">{note}</p>}
      </div>
      <p className="font-heading font-semibold text-[16px] text-neutral-900 tabular-nums">{value}</p>
    </div>
  )
}
