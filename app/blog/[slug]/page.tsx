...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <div>
      ...
      {/* ══ Article header ══ */}
      <section className="bg-white px-5 md:px-8 lg:px-12 pt-10 lg:pt-16">
        <div className="mx-auto max-w-[820px]">
          {/* Back to all */}
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 mb-7 group/back text-[13px] font-heading font-semibold text-vgu-red hover:text-vgu-red-dark transition-colors duration-150"
          >
          ...
          <div className="flex items-center gap-3 mb-5 flex-wrap">
            {post.category && (
              <span className="inline-flex items-center rounded-full bg-vgu-red/10 border border-vgu-red/20 px-3 py-1 text-[11px] font-heading font-bold uppercase tracking-[0.06em] text-vgu-red">
                {post.category}
              </span>
            )}
            <span className="text-[12px] font-body text-neutral-400">{formatDate(post.publishedAt, true)}</span>
            {post.readTime && (
              <>
                <span className="text-[12px] text-neutral-300">·</span>
                <span className="text-[12px] font-body text-neutral-400 inline-flex items-center gap-1">
          ...
          {/* Title */}
          <h1 className="font-heading font-bold text-[32px] tracking-[-0.5px] leading-[1.15] text-neutral-900 mb-6 md:text-[40px] lg:text-[48px]">
            {post.title}
          </h1>

          {/* Excerpt as lede */}
          <p className="text-[18px] font-body leading-[1.65] text-neutral-500 mb-8 lg:text-[20px]">
            {post.excerpt}
          </p>

          {/* Author strip */}
          {post.author && (
            <div className="flex items-center gap-4 pb-10 border-b border-neutral-100">
              <div className={[
                'w-12 h-12 rounded-full overflow-hidden flex-none ring-2 shadow-sm',
                /^(office\s+of|faculty|vgu|vivekananda)/i.test(post.author.name?.trim() ?? '')
                  ? 'ring-vgu-yellow/50'
                  : 'ring-white',
              ].join(' ')}>
                {post.author.avatarUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center font-heading font-bold text-white text-[18px]" style={{ background: 'linear-gradient(135deg, #C04036, #821a12)' }}>
                    {(post.author.name?.charAt(0) ?? 'V').toUpperCase()}
                  </div>
                )}
              </div>
              <div className="min-w-0">
                <div className="font-heading font-semibold text-[15px] text-neutral-900">{post.author.name}</div>
                {post.author.title && (
                  <div className="text-[12px] font-body text-neutral-500">{post.author.title}</div>
                )}
              </div>
            </div>
          )}
      ...
      {/* ══ Cover image ══ */}
      {post.coverUrl && (
        <section className="bg-white px-5 md:px-8 lg:px-12 pt-10">
          <div className="mx-auto max-w-[1120px]">
            <div className="relative aspect-[16/9] rounded-2xl overflow-hidden shadow-[0_24px_60px_rgba(0,0,0,0.14)]">
              <Image
                src={post.coverUrl}
                alt={post.title}
                fill
      ...
      {/* ══ Body ══ */}
      <section className="bg-white px-5 md:px-8 lg:px-12 py-12 lg:py-16">
      ...
      {post.author && (post.author.name || post.author.title) && (
        <section className="bg-neutral-50 px-5 md:px-8 lg:px-12 py-12 lg:py-16">
          <div className="mx-auto max-w-[820px]">
            <div
              className="flex flex-col sm:flex-row items-center gap-5 sm:gap-6 rounded-2xl border border-neutral-200 bg-white p-6 md:p-8 shadow-[0_4px_16px_rgba(0,0,0,0.05)]"
              style={{ background: 'linear-gradient(135deg, #ffffff 60%, rgba(244,215,193,0.30) 100%)' }}
            >
              <div className={[
                ...
                {post.author.avatarUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center font-heading font-bold text-white text-[28px]" style={{ background: 'linear-gradient(135deg, #C04036, #821a12)' }}>
                    {(post.author.name?.charAt(0) ?? 'V').toUpperCase()}
                  </div>
                )}
              </div>
      ...
      {post.relatedPrograms && post.relatedPrograms.length > 0 && (
        <section className="bg-white px-5 md:px-8 lg:px-12 py-12 lg:py-16">
          <div className="mx-auto max-w-[1120px]">
            ...
            <h2 className="font-heading font-bold text-[24px] tracking-[-0.5px] leading-[1.2] text-neutral-900 mb-8 md:text-[28px]">
            ...
            <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {post.relatedPrograms.map(p => (
                <Link
                  key={p.slug}
                  href={`/programs/${p.slug}`}
                  className="group/card flex flex-col rounded-2xl border border-neutral-200 bg-white p-6 hover:border-vgu-red/25 hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(192,64,54,0.10)] transition-all duration-200"
                  style={{ background: 'linear-gradient(135deg, #ffffff 60%, rgba(192,64,54,0.04) 100%)' }}
                >
                  <span className="inline-flex w-fit items-center rounded-full bg-vgu-red/10 border border-vgu-red/20 px-2.5 py-0.5 text-[10px] font-heading font-bold uppercase tracking-[0.06em] text-vgu-red mb-3">
                    {p.level === 'pg' ? 'PG' : p.level === 'ug' ? 'UG' : 'Cert'}
                ...
                <div key={p._id} className="snap-start flex-none w-[82vw] max-w-[320px]">
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-1 p-5">
                      <div className="text-[12px] font-body text-neutral-400 mb-2">
                      ...
                      <p className="text-[14px] font-body text-neutral-500 leading-[1.65] line-clamp-2">
                        {p.excerpt}
                      </p>
                    </div>
                  </Link>
            ...
            <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {related.map(p => (
                <Link
                  key={p._id}
                  href={`/blog/${p.slug}`}
                  className="group/card flex flex-col rounded-2xl border border-neutral-200 bg-white overflow-hidden hover:-translate-y-1.5 hover:shadow-[0_16px_40px_rgba(192,64,54,0.12)] hover:border-vgu-red/20 transition-all duration-200"
                >
                  <div className="relative aspect-[16/9] bg-neutral-100 overflow-hidden">
                    {p.coverUrl ? (
                      <Image
